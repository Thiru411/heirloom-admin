          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="card">
                ...
                <form role="form" id="quickForm" name="myForm" action="<?=base_url()?>inventory-details/update/<?=base64_encode($sk_inventory_id)?>" method="post" enctype="multipart/form-data">
              <div class="card-body">
             <div class="row">
              <div class="col-md-4">
              <div class="form-group">
                  <label>Inventory Title:</label>
                  <input type="text" class="form-control" name="title"value="<?=$title?>" id="title" required placeholder="Title"> 
                </div>  
                <div class="form-group">
                  <label>Mature Size:</label>
                  <input type="text" class="form-control" name="mature_size" value="<?=$matuare_size?>" id="title" required placeholder="50’w x 30’ height"> 
                </div>  
               ...
               <?php  if($categoryDetails){foreach($categoryDetails as $info){ ?>
                  <option value=<?=$info->category_id?><?php if($info->category_id==$category_type){echo "selected";}else{echo "";}?>><?=$info->category_type?></option>
                  <?php }} ?>
                  <!-- <select class="form-control" name="category_type" id="location_id" required  > -->
                 </select>
                </div> 
                ...
                <label>Location:</label>
                  ...
                  <?php }} ?> -->
                  <input type="text" class="form-control float-right"  name="location"  value="<?=$location_name?>">
                ...
                <label>Inventory Image And Video(Max:upto 200MB):(Please Hold Ctrl And Select Images)</label>
                  <input type="file" class="form-control image-size1" name="inventory_image[]" id="event_image" multiple  ><br>
                  <?php if($image){foreach($image as $info){
				            $type = pathinfo($info, PATHINFO_EXTENSION);
                 if($type=='mp4'){?>
                      <video width='100px' height='100px' controls>
                        <source src='<?=$info?>' type='video/mp4'>
                     </video>                         
                   <?php }else{?>
                      <img src="<?=$info?>" width="100px" height="100px">
                      <?php }}} ?>
                </div> 
              ...
              <div class="col-md-4">
                ...
                <div class="form-group">
                  ...
                  <textarea  class="form-control" name="description" value="<?=$description?>"  required ><?=$description?></textarea>
                </div>
                <div class="form-group">
                  <label>Inventory:</label>
                  <input type="text" class="form-control" name="inventory" value="<?=$inventory?>" id="minimum_member" required />
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control float-right" value="<?=$inventory_date?>" name="inventory_date" id="event_date">
                  </div>
                  <!-- /.input group -->
                </div>
              ...
              <div class="form-group">
                  <label>Zone:</label>
                  <input type="text" class="form-control" name="zone" value="<?=$zone?>" id="maximum_member" required/>
                </div>  
                <div class="form-group">                  
                <label>Available:</label>
                  <select class="form-control" name="available" id="location_id"  >
                  <?php if($available=='onsite'){?>
                 <option value="onsite">Onsite</option>
                 <option value="field">Field</option>
                 <option value="heirloom_selection">Heirloom Selection</option>
                 <?php }else if($available=='field'){?>
                  <option value="field">Field</option>
                 <option value="onsite">Onsite</option>
                 <option value="heirloom_selection">Heirloom Selection</option>
                <?php  } else{?>
